...
/**
 * Factory creating {@link TimeInterval} objects from XML. Also contains some auxiliary methods
 * for converting {@link FormulaInterval} to XML. 
 * 
 * @author <a href="mailto:xvejpust@fi.muni.cz">Tomáš Vejpustek</a>
 *
 */
public class FormulaIntervalFactory implements XMLRepresentableFactory<FormulaInterval> {
    ...
    /** Value of type boundary: open -- corresponds to {@link IntervalBoundaryType#OPEN} */
    private static final String OPEN_NAME = "open";
    /** Value of type boundary: closed -- corresponds to {@link IntervalBoundaryType#CLOSED} */
     ...
     */
    private static Element getBound(Document doc, String name, float value, IntervalBoundaryType type) {
    ...
    /**
     * Used by {@link XMLRepresentable#toXML} method to obtain XML element representing interval upper bound.
     * @param doc Parent XML document.
     * @param value Bound value.
     * @param type Bound type.
     * @return XML element representing upper bound.
     */
    public static Element getUpperBound(Document doc, float value, IntervalBoundaryType type) {
        return getBound(doc, UPPER_NAME, value, type);
    }
    
    /**
     * Used by {@link XMLRepresentable#toXML} method to obtain XML element representing interval lower bound.
     * @param doc Parent XML document.
     * @param value Bound value.
     * @param type Bound type
     * @return XML element representing lower bound.
     */
    public static Element getLowerBound(Document doc, float value, IntervalBoundaryType type) {
    ...
    private static IntervalBoundaryType getBoundaryType(Node parent) {
        ...
        if (target.equals(OPEN_NAME)) {
            ...
            assert target.equals(CLOSED_NAME) : target;
    ...
    public FormulaInterval getObject(Element source)
        ...
        for (int index = 0; index < bounds.getLength(); index++) {
            ...
            if (bound.getNodeName().equals(LOWER_NAME)) { //lower bound
                try {
            ...
            } else if (bound.getNodeName().equals(UPPER_NAME)) { //upper bound
                try {
        ...
        if (lower <= 0) {
            throw new XMLFormatException("Lower bound of an interval must be greater or equal to zero.");
        }
        if (upper < lower) {
            throw new XMLFormatException("Upper bound of an interval must be greater (or equal) to lower.");
        }
