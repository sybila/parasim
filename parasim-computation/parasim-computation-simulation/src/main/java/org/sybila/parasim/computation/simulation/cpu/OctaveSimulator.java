...
public class OctaveSimulator implements Simulator<AdaptiveStepConfiguration, SimulatedDataBlock<Trajectory>> {
    ...
    public SimulatedDataBlock<Trajectory> simulate(AdaptiveStepConfiguration configuration, DataBlock<Trajectory> data) {
        ...
        octave.eval(configuration.getOdeSystem().octaveString());
        ...
        for (int i = 0; i < data.size(); i++) {
            trajectories[i] = simulateTrajectory(octave, configuration, data.getTrajectory(i).getLastPoint());
    ...
    private Trajectory simulateTrajectory(OctaveEngine octave, AdaptiveStepConfiguration configuration, Point initialPoint) {
        ...
        octave.eval("y = lsode(\"" + configuration.getOdeSystem().octaveName() + "\", i, t)");
