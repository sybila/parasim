...
class BuchiParser {
    ...
    boolean parseFile(String fileName, OdeSystem ode) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            switch (i) {
                ...
                default:
                    ...
                    } else {   /* q1 -> q2 { guard X>0.001 && Y<3.6; },
                        q1 -> q1 {},
                        q2 -> q2 {}; */
        ...
        for (int i = 0; i < transitions.length; i++) { /* trim the transition array to real sizes */
    ...
    private Evaluable parseExpression(String token, HashMap varIndexes, OdeSystem ode) {
        ...
        if (token.indexOf('(') == -1
                && token.indexOf(')') == -1
                && token.indexOf('&') == -1
                && token.indexOf('|') == -1) { /* detect possible negation */
            ...
            if (error != null || parts == null || parts.length == 0) {
                ...
                return null;
            } else if (parts.length == 1) { /* token must be '(COMPLEX)' or 'not (COMPLEX)' */
                String part = parts[0];
                ...
                } else if (part.charAt(0) == '(' && part.charAt(part.length() - 1) == ')') {   /* strip left and right brackets and whitespace */
